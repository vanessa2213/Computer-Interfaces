# Compute first twenty Fibonacci numbers and put in array, then print reverse
      .data
...
size: .word  20             # size of "array"
      ...
      li   $t2, 6765        # 6765 is first Fib. number
      sw   $t2, 0($t0)      # F[0] = 6765
      li   $t2, 4181        # 4181 is second Fib. number
      sw   $t2, 4($t0)      # F[1] = F[0] = 4181
      addi $t1, $t5, -2     # Counter for loop, will execute (size-2) times
loop: lw   $t3, 0($t0)      # Get value from array F[n]
      lw   $t4, 4($t0)      # Get value from array F[n+1]
      sub  $t2, $t3, $t4    # $t2 = F[n] - F[n+1]
